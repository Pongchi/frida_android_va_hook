...
# ./js 폴더에서 signal로 시작하는 모든 .js 파일을 읽어서 후킹 스크립트에 추가
for filename in os.listdir('./js'):
    if filename.startswith("signal_") and filename.endswith(".js"):
...
def start_frida_server():
    try:
        ...
        time.sleep(1)  # 연결 지연시간 추가

        # Frida 서버 실행
        subprocess.run(['adb', 'shell', '/data/local/tmp/frida-server-64 &'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def set_proxy(ip, port=8080):
    try:
        # Wi-Fi 프록시 설정 명령어 실행
        subprocess.run(['adb', 'shell', 'settings', 'put', 'global', f'http_proxy {ip}:{port}'])
...
def main():
    try:
        ...
        pid = device.spawn(["org.thoughtcrime.securesms"])  # Signal 메신저 패키지명
        ...
        device.resume(pid)  # Signal 메신저 실행 재개
        sys.stdin.read()  # 스크립트 유지
    except frida.ProcessNotFoundError:
        print("프로세스를 찾을 수 없습니다. Signal 앱이 실행 중인지 확인하세요.")
    except Exception as e:
        print(f"오류 발생: {e}")
